...
function AnimatedScene({
  ...
  // Throttle emit updates to at most once every 100ms for performance
  ...
  useFrame(({ clock }) => {
    ...
    if (!isPlaying) {
      if (now - lastEmitTimeRef.current > 0.1) {
    ...
    if (clampedElapsed >= CANDLE_DROP_START) {
      ...
      const candleEase = easeOutCubic(progress);
...
export default function App() {
  ...
  useEffect(() => {
    if (hasAnimationCompleted) {
      setSubtitleText("click on the birthday card");
    }
  ...
  const handleCardToggle = useCallback(
    (id: string) => {
      setActiveCardId((current) => {
        ...
        if (isActive) {
          setSubtitleText("press space bar to blow out the candles");
          return null;
        } else {
          setSubtitleText("click the card to place it back");
          return id;
        }
      });
    },
  ...
  const isScenePlaying = true;

  return (
    <div className="App">
      {hasAnimationCompleted && isCandleLit && null}
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight intensity={0.4} position={[2, 10, 0]} color={[1, 1, 1]} />
          <Environment
          ...
          />
          <EnvironmentBackgroundController intensity={0.3} />
          {fireworksActive && <Fireworks isActive={fireworksActive} origin={[0, 10, 0]} />}
